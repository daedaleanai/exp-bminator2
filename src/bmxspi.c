...
int bmi088_self_test(struct SPIQ *q) {
	...
	bmx_writereg(
			q, ACCEL, BMI08x_ACC_CONF, BMI08x_ACC_CONF_1600HZ);	 // ODR=1.6kHz, continuous sampling mode, “normal mode” (norm_avg4)
	delay(2500);												 // wait > 2ms
...
uint16_t bmx_check_config(struct SPIQ *q, enum BMXFunction bf, const struct bmx_config_t *configs) {
	int bad = 0;
	for (; configs->reg != 0xFF; ++configs) {
		uint8_t	 val = 0;
		...
		if (val != configs->val) {
			printf("Config register 0x%02x check failed, expected 0x%02x, got 0x%02x\n", configs->reg, configs->val, val);
			++bad;
		}
	}
	return bad;
