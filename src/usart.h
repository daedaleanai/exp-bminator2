...
static inline void usart_init(struct UART_Type *usart, int baud) {
	usart->CR1 = 0;
	usart->CR2 = 0;
	usart->CR3 = 0;
...
static inline void usart_tx_irq_handler(struct UART_Type *usart, struct Ringbuffer *rb) {
...
static inline void usart_start(struct UART_Type *usart) { usart->CR1 |= UART_CR1_TXEIE; }

static inline void usart_wait(struct UART_Type *usart) {
