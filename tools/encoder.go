...
func crc_update(crc uint16, data []uint32) uint16 {
	for _, v := range data {
		crc = crc_table[byte(crc>>8)^byte(v>>24)] ^ (crc << 8)
		crc = crc_table[byte(crc>>8)^byte(v>>16)] ^ (crc << 8)
		crc = crc_table[byte(crc>>8)^byte(v>>8)] ^ (crc << 8)
		crc = crc_table[byte(crc>>8)^byte(v)] ^ (crc << 8)
	}
	return crc
}
