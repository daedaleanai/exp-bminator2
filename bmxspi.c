...
static uint16_t bmi_writereg(struct SPIQ *q, enum BMXFunction bf, uint8_t reg, uint8_t val) {
...
static uint16_t bmi_readreg(struct SPIQ *q, enum BMXFunction bf, uint8_t reg, uint8_t *val) {
    struct SPIXmit *x = spiq_head(q);
    if (!x) {
        return 0xffff;
    }
...
uint16_t bmi_accel_poweron(struct SPIQ *q) {
    uint8_t dum = 0;
    uint16_t r = bmi_readreg(q, ACCEL, BMI08x_ACC_CHIP_ID, &dum);
    if (r != 0) {
        return r;
    }
    r = bmi_writereg(q, ACCEL, BMI08x_ACC_PWR_CTRL, 0x04);
    if (r != 0) {
        return r;
    }
    delay(50000);
    return bmi_writereg(q, ACCEL, BMI08x_ACC_PWR_CONF, 0x0);
}
...
int bmi088_self_test(struct SPIQ *q) {
    ...
    uint16_t r = bmi_readreg(q, GYRO, BMI08x_GYRO_CHIP_ID, &chip_id);
    if ((r != 0) || (chip_id != 0x0f)) {
    ...
    bmi_writereg(q, GYRO, BMI08x_GYRO_SELF_TEST, 0x01);  // trigger self test in gyro

    r = bmi_readreg(q, ACCEL, BMI08x_ACC_CHIP_ID, &chip_id);
    if (r == 0) {
        r = bmi_readreg(q, ACCEL, BMI08x_ACC_ERR_REG, &err_code);
    }
    ...
    bmi_writereg(q, ACCEL, BMI08x_ACC_RANGE, BMI085_ACC_RANGE_16G);  // ±16g range: +2^15 ~ 16G -> 1G = 2^11
    bmi_writereg(q, ACCEL, BMI08x_ACC_CONF,  BMI08x_ACC_CONF_1600HZ);  // ODR=1.6kHz, continuous sampling mode, “normal mode” (norm_avg4)
    delay(2500);  // wait > 2ms
    ...
    printf(" positive");
    bmi_writereg(q, ACCEL, BMI08x_ACC_SELF_TEST, 0x0D);  // Enable the positive self-test polarity
    ...
    printf(" negative");
    bmi_writereg(q, ACCEL, BMI08x_ACC_SELF_TEST, 0x09);  // Enable the negative self-test polarity
    delay(100000);  // Wait for > 50ms
    ...
    bmi_writereg(q, ACCEL, BMI08x_ACC_SELF_TEST, 0x0);  // disable self-test
    ...
    r = bmi_readreg(q, GYRO, BMI08x_GYRO_SELF_TEST, &err_code);
    ...
    r = bmi_writereg(q, GYRO, BMI08x_GYRO_SOFTRESET, 0xB6);
    if (r) {
        printf("gyro reset spi error: %x\n", r);
        ++err;
    }

    r = bmi_writereg(q, ACCEL, BMI08x_ACC_SOFTRESET, 0xB6);
    if (r) {
...
int bme280_self_test(struct SPIQ *q, struct LinearisationParameters *bmeParam) {
    uint16_t r = bmi_writereg(q, NONE, BME280_REG_RESET, BME280_RESET);
    if (r != 0) {
    ...
    uint8_t chip_id = 0;
    r = bmi_readreg(q, NONE, BME280_REG_ID, &chip_id);
...
uint16_t bmx_config(struct SPIQ *q, enum BMXFunction bf, struct bmx_config_t *configs) {
    for (; configs->reg != 0xFF; ++configs) {
        uint16_t r = bmi_writereg(q, bf, configs->reg, configs->val);
        if (r != 0) {
            return r;
        }
    }
